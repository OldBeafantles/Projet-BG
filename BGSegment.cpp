...
BGSegment::BGSegment(BGPoint _p1, BGPoint _p2)
{
    m_p1 = _p1;
    m_p2 = _p2;
}
...
bool BGSegment::areSecant(const BGSegment& _s1, const BGSegment& _s2)
{
    ...
    if (_s1.isVertical() || _s2.isVertical()) //Dans le cas où au moins un des segments est vertical
    {
        ...
        if (BGSegment::isXIn(sVertical.P1().x(), s.P1().x(), s.P2().x()))
        {
            ...
            double posX1 = sVertical.P1().x() + diffPoints;
            if (posX1 < 0) posX1 = 0;
            double posX2 = sVertical.P2().x() + diffPoints;
            if (posX2 < 0) posX2 = 0;
            ...
            BGSegment resultSegment = BGSegment(BGPoint(posX1, posY1), BGPoint(posX2, posY2));
